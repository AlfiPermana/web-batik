<div class="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <p class="text-xs text-gray-600 leading-relaxed">
            ...
            @php
                ...
                if (!empty($subdistrict)) {
                    $subDistrictName = collect($subdistricts)->firstWhere('id', $subdistrict)['name'] ?? $subdistrict;
                    $locationParts[] = $subDistrictName;
                }
                if (!empty($district)) {
                    $districtName = collect($districts)->firstWhere('id', $district)['name'] ?? $district;
                    $locationParts[] = $districtName;
                }
                if (!empty($city)) {
                    $cityName = collect($cities)->firstWhere('id', $city)['name'] ?? $city;
                    $locationParts[] = $cityName;
                }
                if (!empty($province)) {
                    $provinceName = collect($provinces)->firstWhere('id', $province)['name'] ?? $province;
                    $locationParts[] = $provinceName;
                }
                $locationStr = implode(', ', $locationParts);
            @endphp
            {{ $locationStr }} {{ $postalCode }}<br>
            {{ $phoneNumber }}
        </p>
    </div>
    ...
    @if ($selectedShippingService)
        <div class="mb-4 pb-4 border-b border-gray-200">
            ...
            <p class="text-xs text-gray-600 leading-relaxed">
                <strong>{{ $selectedShippingService['courier'] }} - {{ $selectedShippingService['service'] }}</strong><br>
                {{ $selectedShippingService['description'] }}<br>
                <span class="text-gray-500">Estimasi: {{ $selectedShippingService['estimation'] }}</span>
            </p>
        </div>
    @endif
