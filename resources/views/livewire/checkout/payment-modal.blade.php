@if($showTripayModal)
    ...
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm max-h-[85vh] overflow-hidden flex flex-col"
             wire:poll.keep-alive.3s="checkPaymentStatus">
            
            <!-- Modal Header -->
            <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200"
                 @if($paymentStatus === 'paid') style="background: linear-gradient(to right, #10b981, #059669);"
                 @elseif($paymentStatus === 'failed') style="background: linear-gradient(to right, #ef4444, #dc2626);"
                 @elseif($paymentStatus === 'expired') style="background: linear-gradient(to right, #f59e0b, #d97706);"
                 @elseif($paymentStatus === 'refunded') style="background: linear-gradient(to right, #3b82f6, #2563eb);"
                 @endif>
                <h2 class="text-lg font-bold" 
                    @if(in_array($paymentStatus, ['paid','failed','expired','refunded'], true)) style="color: white;" @else style="color: #8B4513;" @endif>
                ...
                <button type="button"
                        ...
                        @if(in_array($paymentStatus, ['paid','failed','expired','refunded'], true)) style="color: white;" @else class="text-gray-400 hover:text-gray-600" @endif>
            ...
            <div class="flex-1 overflow-y-auto p-4">
                    ...
                    <div class="py-6">
                        ...
                        <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-1">Termasuk Admin Fee Tripay: Rp {{ number_format($tripayFee ?? 0, 0, ',', '.') }}</p>
                            </div>
                        </div>
                ...
                @elseif(in_array($paymentStatus, ['failed','expired','refunded'], true))
                    @php
                        $expiredAtLabel = null;
                        try {
                            if (is_array($tripayPaymentData) && !empty($tripayPaymentData['expired_time']) && is_numeric($tripayPaymentData['expired_time'])) {
                                $expiredAtLabel = \Illuminate\Support\Carbon::createFromTimestamp((int) $tripayPaymentData['expired_time'])->format('d-m-Y H:i:s');
                            } elseif (is_array($tripayPaymentData) && !empty($tripayPaymentData['expired_at'])) {
                                $expiredAtLabel = \Illuminate\Support\Carbon::parse($tripayPaymentData['expired_at'])->format('d-m-Y H:i:s');
                            }
                        } catch (\Throwable $e) {
                            $expiredAtLabel = null;
                        }
                    ...
                    <div class="py-6">
                        <div class="mb-4 p-3 rounded-lg border"
                             @if($paymentStatus === 'failed') style="background-color: #fef2f2; border-color: #fecaca;"
                             @elseif($paymentStatus === 'expired') style="background-color: #fff7ed; border-color: #fed7aa;"
                             @else style="background-color: #eff6ff; border-color: #bfdbfe;" @endif>
                            <p class="text-sm font-semibold"
                               @if($paymentStatus === 'failed') style="color: #991b1b;"
                               @elseif($paymentStatus === 'expired') style="color: #9a3412;"
                               @else style="color: #1e40af;" @endif>
                                {{ $paymentStatusMessage ?? 'Status pembayaran diperbarui.' }}
                            </p>
                            @if($expiredAtLabel)
                                <p class="text-xs text-gray-600 mt-2">Batas Pembayaran: <span class="font-semibold">{{ $expiredAtLabel }}</span></p>
                            @endif
                        </div>
                        ...
                        <div class="p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-mono font-semibold text-gray-900 break-all">{{ $tripayReferenceCode ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between text-sm mt-2">
                                <span class="text-gray-700">Total</span>
                                <span class="font-bold" style="color: #8B4513;">Rp {{ number_format($totalAmount ?? 0, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>
                ...
                @elseif($tripayPaymentUrl && $tripayPaymentData)
                    ...
                    <div class="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        ...
                        @if($expiredAtLabel)
                            <p class="text-xs text-gray-700 mt-2">Batas Pembayaran: <span class="font-semibold">{{ $expiredAtLabel }}</span></p>
                        @endif
                    ...
                    <div class="mb-4 p-3 bg-orange-50 rounded-lg border border-orange-200">
                        ...
                        <div class="flex justify-between text-sm mb-2 pb-2 border-b border-orange-300">
                            ...
                            <span class="text-orange-600">Rp {{ number_format($tripayFee ?? 0, 0, ',', '.') }}</span>
                        </div>
                ...
                @else
                    ...
                    <div class="py-10">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <p class="text-gray-600 font-semibold text-sm mb-4">Mempersiapkan Pembayaran...</p>
                        </div>

                        <!-- Ringkasan biar user langsung lihat admin fee -->
                        <div class="p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <div class="flex justify-between text-sm mb-2">
                                <span class="text-gray-700">Subtotal</span>
                            ...
                            <div class="flex justify-between text-sm mb-2 pb-2 border-b border-gray-300">
                                <span class="text-gray-700">Admin Fee Tripay</span>
                                <span>Rp {{ number_format($tripayFee ?? 0, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between font-bold">
                                <span style="color: #8B4513;">Total</span>
                                <span style="color: #8B4513;">Rp {{ number_format($totalAmount ?? 0, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>
                @endif
