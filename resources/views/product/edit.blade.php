<x-layouts.app :title="__('Edit Product')">
    <div class="flex h-full w-full flex-1 flex-col gap-4">
        <div class="mb-2">
            ...
            <flux:subheading>Update product information</flux:subheading>
        </div>
            ...
            <form action="{{ route('admin.product.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    <flux:field>
                        ...
                        <img src="{{ asset('storage/' . $product->photo) }}"
                             alt="{{ $product->title }}"
                             class="w-48 h-48 object-cover rounded-xl border border-neutral-200 dark:border-neutral-700 mb-3">
                    </flux:field>
                </div>
                ...
                <div>
                    <div class="flex justify-between items-center mb-3">
                        <flux:label>Product Sizes</flux:label>
                    ...
                    <div id="sizes-container" class="space-y-3">
                        @foreach($product->sizes as $index => $size)
                            <div class="flex gap-3 items-start" id="size-row-{{ $index }}">
                                <div class="flex-1">
                                    <input
                                        type="text"
                                        name="sizes[{{ $index }}][name]"
                                        value="{{ old('sizes.' . $index . '.name', $size->size) }}"
                                ...
                                <div class="w-40">
                                    <input
                                        ...
                                        value="{{ old('sizes.' . $index . '.stock', $size->stock ?? 0) }}"
                                        placeholder="Stock"
                                        min="0"
                                        required
                                        class="w-full px-3 py-2 border border-zinc-300 dark:border-zinc-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-zinc-800 dark:text-zinc-100"
                ...
                <div>
                    <flux:label>Existing Images</flux:label>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3">
                        @foreach($product->images as $image)
                            <div class="relative group">
                                <img src="{{ asset('storage/' . $image->photo) }}"
                                     alt="Product Image"
                                     class="w-full h-32 object-cover rounded-lg border border-neutral-200 dark:border-neutral-700">
                                <label class="absolute top-2 right-2 bg-white dark:bg-zinc-800 rounded-lg p-2 cursor-pointer hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                                    <input type="checkbox" name="delete_images[]" value="{{ $image->id }}" class="sr-only peer">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-zinc-400 peer-checked:text-red-600" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" />
                                    </svg>
                                </label>
                            </div>
                    ...
                    <flux:description class="mt-2">Check images to delete them</flux:description>
                </div>
                @endif
                ...
                <div>
                    <div class="flex justify-between items-center mb-3">
                        <flux:label>Add New Images (Optional)</flux:label>
